...
) => ensureUserExists(userPromise, res).then((user) => user ? onUser(user) : null);
...
export const handleOutcome = (
  ...
  match(outcome)
    .with({ _tag: "Expired" }, () => respondVerificationError(res, 400, "code_expired"))
    .with({ _tag: "TooManyAttempts" }, () => respondVerificationError(res, 400, "too_many_attempts"))
