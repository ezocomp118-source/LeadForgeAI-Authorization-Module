...
// COMPLEXITY: O(1)
export const computeExpiresAt = (nowMs: number, ttlMinutes: number): Date =>
	new Date(nowMs + ttlMinutes * 60_000);
...
// COMPLEXITY: O(1)
export const isRateLimited = (issuedInWindow: number, limit: number): boolean =>
	issuedInWindow >= limit;
